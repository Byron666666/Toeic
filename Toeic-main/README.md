...
如果要使用 PWA 安裝與離線快取，請用本機伺服器或 HTTPS 網址開啟，因為瀏覽器不會在 `file://` 頁面啟用 service worker。
...
- `manifest.webmanifest`：App 名稱、顏色、啟動畫面與安裝圖示設定
- `sw.js`：快取主要 HTML、CSS、JS、單字資料與圖示，支援離線開啟
- `icons/`：主畫面與瀏覽器使用的 app icon

安裝方式：

- iPhone / iPad：用 Safari 開啟網站，按分享，選「加入主畫面」。
- Android / Chrome：開啟網站後使用瀏覽器的「安裝應用程式」或「加入主畫面」。
